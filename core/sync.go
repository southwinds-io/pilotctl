...
func applyLinks(links *DiffLinkReport, api *API) error {
	for _, l := range links.Added {
		r, err := api.iLink.PutLink(&ilink.Link{
			Key:          l.key(),
			StartItemKey: l.From,
			EndItemKey:   l.To,
...
func applyItems(d *DiffReport, itemType string, api *API) error {
	if d.Added != nil {
		for _, i := range d.Added.Values {
			r, err := api.iLink.PutItem(&ilink.Item{
				Key:         i.Key,
				Name:        i.Name,
	...
	if d.Removed != nil {
		for _, i := range d.Removed.Values {
			r, err := api.iLink.DeleteItem(&ilink.Item{Key: i.Key})
			if r.Error {
				return fmt.Errorf(r.Message)
			}
			if err != nil {
...
func loadDb(api *API, itemType string) (*infoList, error) {
	list := new(infoList)
	items, err := api.iLink.GetItemsByType(itemType)
	if err != nil {
		return nil, err
	}
