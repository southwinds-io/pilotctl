...
func (r *API) AuthenticatePilot(token string) *h.UserPrincipal {
	if len(token) == 0 {
...
func (r *API) AuthenticateUser(request http.Request) *h.UserPrincipal {
...
func (r *API) AuthenticateActivationSvc(request http.Request) *h.UserPrincipal {
	cf := &Conf{}
	user, pwd := h.ParseBasicToken(request)
